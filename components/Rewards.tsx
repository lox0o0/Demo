...
function RewardCard({ reward, onClaim }: { reward: Reward; onClaim: () => void }) {
  ...
  return (
    <div
      className={`bg-white/10 backdrop-blur-md border-l-4 ${borderColor} border border-white/30 rounded-lg p-4 hover:shadow-xl transition-all ${opacity} ${
    ...
    >
      <div className="flex items-start justify-between mb-3">
        <div className="flex items-center gap-3">
          <div className={`w-10 h-10 rounded-lg flex items-center justify-center ${
            isClaimable ? "bg-amber-500/30 border-2 border-amber-500/60" : "bg-nrl-green/30 border-2 border-nrl-green/60"
          }`}>
            <Icon className={`w-5 h-5 ${isClaimable ? "text-amber-300" : "text-nrl-green"}`} />
          </div>
          <div className="flex-1">
            <div className="flex items-center gap-2 mb-1 flex-wrap">
              <h3 className="text-base font-bold text-white">{reward.title}</h3>
              {reward.ageRestricted && (
                <span className="px-2 py-0.5 bg-red-500/30 border border-red-500/60 rounded text-xs font-semibold text-red-300">
                  18+
                </span>
              )}
              {reward.rewardType === "tier" && (
                <span className="px-2 py-0.5 bg-amber-500/30 border border-amber-500/60 rounded text-xs font-semibold text-amber-200">
                  TIER REWARD
                </span>
              )}
              {reward.rewardType === "experience" && (
                <span className="px-2 py-0.5 bg-blue-500/30 border border-blue-500/60 rounded text-xs font-semibold text-blue-200">
                  EXPERIENCE
                </span>
              )}
              {!isClaimable && (
                <span className="px-2 py-0.5 bg-nrl-green/30 border border-nrl-green/60 rounded text-xs font-semibold text-nrl-green flex items-center gap-1">
                  CLAIMED <Check className="w-3.5 h-3.5" />
                </span>
              )}
            </div>
            <p className="text-sm text-white/90 whitespace-pre-line leading-relaxed">{reward.description}</p>
          </div>
        </div>
      </div>
      ...
      {reward.status && (
        <div className="text-xs text-white/80 whitespace-pre-line mb-2 leading-relaxed">
          {reward.status}
        </div>
      )}
      ...
      {isClaimable && (
        <button
          onClick={onClaim}
          className="w-full mt-3 px-4 py-2.5 bg-nrl-green/30 hover:bg-nrl-green/40 border-2 border-nrl-green/60 rounded-lg text-nrl-green hover:text-white font-bold text-sm transition-all shadow-lg hover:shadow-nrl-green/20"
        >
          {reward.ageRestricted
      ...
      {isClaimable && reward.ageRestricted && (
        <p className="text-xs text-white/60 mt-2">Must be 18+ to claim this reward</p>
      )}
    </div>
  );
}
