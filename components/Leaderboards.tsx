...
export default function Leaderboards({ user }: LeaderboardsProps) {
  ...
  const renderMovement = (movement: number | null) => {
    if (movement === null) {
      return <span className="text-gray-500 flex items-center gap-1"><Minus className="w-4 h-4" /> —</span>;
    }
    if (movement > 0) {
      return <span className="text-emerald-400 flex items-center gap-1"><TrendingUp className="w-4 h-4" /> ↑ {movement}</span>;
    }
    return <span className="text-red-400 flex items-center gap-1"><TrendingDown className="w-4 h-4" /> ↓ {Math.abs(movement)}</span>;
  };
  ...
  return (
    <div className="min-h-screen bg-[#0d0d0d] text-white">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-24 pb-8">
        ...
        <p className="text-nrl-text-secondary text-lg">See how you stack up against other fans</p>
      </div>
...
function UserPositionCard({ 
  ...
  return (
    <div className="mb-6 p-6 rounded-xl backdrop-blur-md bg-background/45 border-2 border-nrl-green/50 shadow-lg">
      <div className="flex items-center justify-between mb-4">
        <div>
          <h3 className="text-lg font-bold text-white mb-1">Your Position</h3>
          <div className="flex items-center gap-3">
            <span className="text-2xl font-bold text-nrl-green">#{rank}</span>
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-white/10">
                <th className="text-left py-4 px-6 text-nrl-text-secondary font-semibold">Rank</th>
                <th className="text-left py-4 px-6 text-nrl-text-secondary font-semibold">Player</th>
                <th className="text-right py-4 px-6 text-nrl-text-secondary font-semibold">Correct</th>
                <th className="text-right py-4 px-6 text-nrl-text-secondary font-semibold">Accuracy</th>
                <th className="text-right py-4 px-6 text-nrl-text-secondary font-semibold">Streak</th>
                <th className="text-center py-4 px-6 text-nrl-text-secondary font-semibold">Movement</th>
              </tr>
            </thead>
            <tbody>
  ...
  return (
    <>
      ...
      <UserPositionCard
        ...
        customContent={
          <div className="text-right">
            <div className="text-lg font-bold text-white">{userTotalPts.toLocaleString()} pts</div>
            <div className="text-sm text-nrl-text-secondary mt-1">Week: {userWeekPts} pts</div>
            <div className="text-sm text-nrl-text-secondary">Captain: {userCaptain}</div>
          </div>
        }
      />
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-white/10">
                <th className="text-left py-4 px-6 text-nrl-text-secondary font-semibold">Rank</th>
                <th className="text-left py-4 px-6 text-nrl-text-secondary font-semibold">Player</th>
                <th className="text-right py-4 px-6 text-nrl-text-secondary font-semibold">Total Pts</th>
                <th className="text-right py-4 px-6 text-nrl-text-secondary font-semibold">Week Pts</th>
                <th className="text-left py-4 px-6 text-nrl-text-secondary font-semibold">Captain</th>
                <th className="text-center py-4 px-6 text-nrl-text-secondary font-semibold">Movement</th>
              </tr>
            </thead>
            <tbody>
